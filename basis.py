...
def stock_historical_pnl(spot_file, date):
    ...
    for ticker in ticker_set:
        with TsTickData() as tsl:
            price = tsl.getHistoricalPrice(ticker, date)
        historical_price_df = historical_price_df.append(pd.DataFrame([[ticker, price], ], columns=["ticker", "historical_price"]))
        print(j)
        j += 1
    spot_df = pd.merge(spot_df, historical_price_df, left_on="证券代码", right_on="ticker", how="outer")
    spot_df["pnl"] = (spot_df["historical_price"].sub(spot_df["成本价"])).mul(spot_df["股份余额"])
...
if __name__ == "__main__":
    ...
    # spot_df, future_df = makeDf("spot_1105.xlsx", "future_1105.xls", "加仓")
    # print(calculate_basis(spot_df, future_df))
    stock_historical_pnl("spot_1104.xlsx", "20191105")
